/**
 * @file 更新代码块与链接纯透明样式.js
 * @description 彻底移除代码块、引用链接胶囊与文件变更条的高斯模糊雾化处理，将其重构为纯透明极致通透样式，保留精致微光边框
 */
...
/**
 * 构建代码块与引用链接彻底纯透明的主题 CSS
 *
 * @function generatePureTransparentCss
 * @param {string} baseCss - 原主题 CSS 内容（包含 Base64 背景数据流）
 * @returns {string} 完整的最新纯透明主题 CSS
 * @throws {Error} 若未找到分段标记则抛出异常
 */
function generatePureTransparentCss(baseCss) {
    const p2Marker = '/* ================= 2.';
    const p3Marker = '/* ================= 3.';
    ...
    const section1 = `/**
 * @name        晨雾森林毛玻璃主题
 * @description 基于 wallhaven 晨雾松林丁达尔光壁纸定制的高级透明主题 (代码块纯透明通透版)
...
`;
    ...
    // 2. 构建第 3~12 节（代码块、引用链接彻底纯透明，移除雾化模糊）
...
/**
 * 主执行流程：将代码块与引用链接调整为纯透明无雾化样式，并同步到工程与系统主题
 *
 * @function main
 * @returns {void}
 */
function main() {
    ...
    console.log('[2/3] 生成纯透明极致通透规则（彻底去除代码块雾化）的新主题...');
    const updatedCss = generatePureTransparentCss(rawCss);
